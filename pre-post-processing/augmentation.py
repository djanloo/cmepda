...
class Augment:
        ...
        for file in dataset:
            for key in aug_types:
                ...
                new_record["time_series"] = np.array(
                    [
                        self.augment_matrix(_)[key]
                        for _ in file["time_series"].reshape(-1, 9, 9)
                    ]
                )
    ...
    def augment_matrix(self, matrix):
        ...
        for key in new_record.keys():
            if "rot" in key:
                new_record[key][0] = self.rotate_matrix(
                    matrix, angle=new_record[key][1]
                )
            if "flip" in key:
                new_record[key][0] = self.flip_matrix(
                    matrix, flip_mode=new_record[key][1]
                )

...
for i, delta_quant in enumerate(delta_quants):
    for segment in track(range(N-1)):
        ...
        down, up = np.percentile(predictions[mask], [50 - delta_quant/2, 50 + delta_quant/2])
...
for i in range(len(delta_quants)-1):
    plt.fill_between(vals[:-1], downs[i,:-1], downs[i+1, :-1], 
                    color=colormap(1-delta_quants[i]/100.), alpha=0.5, label=f"{delta_quants[i]}")
    plt.fill_between(vals[:-1], ups[i,:-1], ups[i+1, :-1], 
                    color=colormap(1-delta_quants[i]/100.), alpha=0.5)
plt.fill_between(vals[:-1], ups[-1,:-1], downs[-1,:-1], label=f"{delta_quants[-1]}",color=colormap(1-delta_quants[-1]/100.), alpha=0.5)
plt.legend()

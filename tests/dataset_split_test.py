...
class dataSplitTest(unittest.TestCase):
    ...
    def test_split(self):
        split_dataset(
            (self.time_of_arrival, self.time_series, self.outcomes),
            "fake_dataset",
            ["toa", "time_series", "outcomes"],
            10,
            parent=PARENT_FOLDER,  # Required for permission issues
        )


class datafeederTest(unittest.TestCase):
    def setUp(self):
        self.test_data = DataFeeder(
            join(PARENT_FOLDER, "fake_dataset_splitted_data")
        ).feed()
